...
select invoice_number,supplier_name,site_id,invoice_id,invoice_date,invoice_tot_amount,internal_status from ap_invoice_table \
...
SELECT INVOICE_ID, \
 ...
 WHERE invoice_id = ? \
   ...
   AND  (rowid,employee_number) IN (select max(rowid),employee_number from ap_audit_invoice_table \
