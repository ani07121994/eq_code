...
/*	public static final String wft_query = "Select textattribute1||textattribute6 invSuppNameAP from SOAPROD12C_SOAINFRA.wftask where state like 'ASSIGNED%' and TITLE ='AP Invoice Error'\r\n"
	...
	public static void main(String[] args) throws SQLException {
		...
		Date date = new Date();
		System.out.println("=========================== Program has started for JetStar: ===========================" + dateFormat.format(date));
		...
		{	String q = query;
			...
			while (rs.next()) {
				if (flag.equals("apException")){
					...
					captureidMap_apException.put(rs.getString("captureBachid"),rs.getString("status")); // capture_id
					//captureidMap_apException.put(rs.getString("ddocname"),rs.getString("status")); // ipm id
				
				}
			
				else if(flag.equals("certifier"))
				{
					captureidList_certifier.add(rs.getString("captureBachid")); // capture_id
					imaging_captureid_list.add(captureidList_certifier); // capture_id
					
					/*captureidList_certifier.add("3."+rs.getString("ddocname"));  // ipm id
					imaging_captureid_list.add(captureidList_certifier);   // ipm id
*/					
				}
	...
	public static ArrayList<ArrayList<String>> getKeys (MultiValueMap map, String value)
	{
		...
		for (int j = 0; j < list.size(); j++) {
		 if (list.get(j).equals("Invoice Updated by APGroup")) {
			 firstList.add(mapEntry.getKey().toString()); // capture_id 
			 //firstList.add("3."+mapEntry.getKey().toString()); // ipm id
			 //return mapEntry.getKey().toString();
			 
		 }else {
			 secondList.add(mapEntry.getKey().toString()); // capture_id
			 //secondList.add("3."+mapEntry.getKey().toString()); // ipm id
		...
		try {
			...
			if (capture_id_List.size() > 999) {
				...
				for (int i = 0; i < parts.size(); i++) {
					...
					String query = "select invoice_number,modified_on,modified_by,site_id,header_attr7,internal_status,supplier_name, doc_id from js_ap_invoice_table where modified_on > '16-OCT-22' and header_attr7  in ("; // doc_id, header_attr7
					...
					try (Connection connection = dbUtil.getConnection(dbParams_BPM);
						...
						while (rs.next()) {
							if (flag.equals("apException")) {
								if ((!rs.getString("internal_status").contains("INVOICE IN AP EXCEPTIONS"))
										&& (!rs.getString("internal_status").toUpperCase()
												.contains("Invoice Rejected By Certifier".toUpperCase())) && (!rs.getString("internal_status").contains("DUPLICATE INVOICE REJECTED BY SYSTEM"))) {
									//result_List.add(rs.getString("doc_id"));
									result_List.add(rs.getString("header_attr7"));
									result_List_status.add(rs.getString("internal_status"));
								}
							} else if (flag.equals("certifier")) {
								if ((!rs.getString("internal_status").contains("INVOICE PENDING FOR CERTIFICATION")) && (!rs.getString("internal_status").contains("DUPLICATE INVOICE REJECTED BY SYSTEM"))) {
									//result_List.add(rs.getString("doc_id"));
									result_List.add(rs.getString("header_attr7"));
									result_List_status.add(rs.getString("internal_status"));
								}
							...
							//result_list_all.add(rs.getString("doc_id"));
