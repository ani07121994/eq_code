...
import qantas.integration.util.QFEQIFBase;
 ...
 *
 */
public class QFEQIFFtpClient implements QFEQIFBase
{
  private static final Logger logger = QFEQIFLogger.getLogger(QFEQIFFtpClient.class.getName());
  private final String CLASS_NAME = getClass().getName();

  private String m_server = QFEQIFConstant.BLANK;
  private String m_user = QFEQIFConstant.BLANK;
  ...
  */
  public boolean connect(String p_server, int p_port, String p_user, String p_password)
  {
    ...
    try
    {
      ...
      m_reader = new BufferedReader(new InputStreamReader(m_socket.getInputStream(), QFEQIFConstant.ENCODING_UTF8));
      m_writer = new PrintWriter(new OutputStreamWriter(m_socket.getOutputStream(), QFEQIFConstant.ENCODING_UTF8), true);
  ...
  */
  private String getReplyCode() throws IOException
  {
    ...
    if (reply.charAt(3) == QFEQIFConstant.DASH_CHAR)
    {
      ...
      while (!complete)
      {
        ...
        if (line.length() > 3 &&
          ...
          line.charAt(3) == QFEQIFConstant.SPACE_CHAR)
        {
  ...
  */
  protected String issueCommand(String p_command) throws IOException
  {
    ...
    m_writer.write(p_command + QFEQIFConstant.NEW_LINE);
    m_writer.flush();
  ...
  */
  public Socket getActiveDataSocket(String p_cmd) throws IOException
  {
    ...
    String cmd = new StringBuffer (FTP_CMD_PORT)
                .append (toUnsignedShort (hostBytes[0])) .append (QFEQIFConstant.COMMA)
                .append (toUnsignedShort (hostBytes[1])) .append (QFEQIFConstant.COMMA)
                .append (toUnsignedShort (hostBytes[2])) .append (QFEQIFConstant.COMMA)
                .append (toUnsignedShort (hostBytes[3])) .append (QFEQIFConstant.COMMA)
                .append (toUnsignedShort (portBytes[0])) .append (QFEQIFConstant.COMMA)
