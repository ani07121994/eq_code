...
public class QFEQIFDirectFileLockingUtility {
	...
	private static final Logger logger = SFTPLogger.getLogger(FileLockingUtility.class.getName());
	...
	public boolean getLock(String filename, String time, String ifId){
		...
		try{
			...
			ps   = conn.prepareStatement("SELECT COUNT(*) FROM QFEQIF_SFTP_DIRECT_INTERFACES WHERE FILE_NAME = ? AND CREATED_DATE = ? AND STATUS IN ('WIP','SUCCESS')");
			...
			if(rowCount > 0){
				...
				ps = conn.prepareStatement("INSERT INTO QFEQIF_SFTP_DIRECT_INTERFACES(IF_ID, FILE_NAME, CREATED_DATE, STATUS, POLL_DATE) VALUES(?, ?, ?, ?, ?)");
				ps.setString   (1, ifId);
	...
	public boolean releaseLock(String filename, String time, String status){
		...
		try{
			...
			ps   = conn.prepareStatement("UPDATE QFEQIF_SFTP_DIRECT_INTERFACES SET STATUS = ? WHERE FILE_NAME = ? AND CREATED_DATE = ? AND STATUS = 'WIP'");
	...
	public void deleteFileEntry(String filename, String time, String ifId){
		...
		try{
			...
			ps   = conn.prepareStatement("DELETE FROM QFEQIF_SFTP_DIRECT_INTERFACES WHERE FILE_NAME = ? AND CREATED_DATE = ? AND IF_ID = ?");
			ps.setString(1, filename);
			ps.setString(2, time);
			ps.setString(3, ifId);
			ps.executeUpdate();
