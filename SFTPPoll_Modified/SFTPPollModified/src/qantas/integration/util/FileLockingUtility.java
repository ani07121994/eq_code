...
public class FileLockingUtility {
	...
	public boolean getLock(String filename, String time, String ifId){
		...
		try{
			...
			ps   = conn.prepareStatement("SELECT COUNT(*) FROM QFEQIF_SFTP_DETAIL WHERE FILE_NAME = ? AND CREATED_DATE = ? AND STATUS = 'WIP'");
			...
			if(rowCount > 0){
				...
				ps = conn.prepareStatement("INSERT INTO QFEQIF_SFTP_DETAIL(IF_ID, FILE_NAME, CREATED_DATE, STATUS, POLL_DATE) VALUES(?, ?, ?, ?, ?)");
				ps.setString   (1, ifId);
	...
	public boolean releaseLock(String filename, String time, String status){
		...
		try{
			...
			ps   = conn.prepareStatement("UPDATE QFEQIF_SFTP_DETAIL SET STATUS = ? WHERE FILE_NAME = ? AND CREATED_DATE = ? AND STATUS = 'WIP'");
	...
	public void deleteFileEntry(String filename, String time, String ifId){
		...
		try{
			...
			ps   = conn.prepareStatement("DELETE FROM QFEQIF_SFTP_DETAIL WHERE FILE_NAME = ? AND CREATED_DATE = ? AND IF_ID = ?");
			ps.setString(1, filename);
			ps.setString(2, time);
			ps.setString(3, ifId);
			ps.executeUpdate();
